...
const MARQUEE_ITEMS = [
  "Molecular Filtration","14-Stage Purification","99.9% Purity","Smart TDS Monitor","Alkaline Infused","Zero Waste Architecture","UV-C Sterilization",
  "Molecular Filtration","14-Stage Purification","99.9% Purity","Smart TDS Monitor","Alkaline Infused","Zero Waste Architecture","UV-C Sterilization",
];
...
const TESTIMONIALS = [
  {
    ...
    avatar: "PR",
    rating: 5,
    quote: "I've tried every premium water purifier on the market. AquaPura is in a different league entirely — the water tastes like it was born in a glacier.",
    model: "Elite X-1",
  },
  {
    name: "Arjun Mehta",
...
function ModelElite() {
  return (
    <svg className="ro-model-svg" width="160" height="260" viewBox="0 0 160 260" fill="none">
      <defs>
        <linearGradient id="grad1" x1="30" y1="40" x2="130" y2="220" gradientUnits="userSpaceOnUse">
          <stop offset="0%" stopColor="#ddeeff"/><stop offset="100%" stopColor="#a8c8ff"/>
        </linearGradient>
        <linearGradient id="grad2" x1="45" y1="28" x2="115" y2="44" gradientUnits="userSpaceOnUse">
          <stop offset="0%" stopColor="#b8d0ff"/><stop offset="100%" stopColor="#7aaaff"/>
        </linearGradient>
        <linearGradient id="grad3" x1="68" y1="220" x2="92" y2="250" gradientUnits="userSpaceOnUse">
          <stop offset="0%" stopColor="#a0c0ff"/><stop offset="100%" stopColor="#6090ff"/>
        </linearGradient>
      </defs>
      ...
      {[80,96,112,128].map((y,i)=>(
        <line key={i} x1="50" y1={y} x2="110" y2={y} stroke={`rgba(0,87,255,${0.4-i*0.08})`} strokeWidth="2" strokeLinecap="round"/>
      ))}
...
function ModelElement() {
  return (
    <svg className="ro-model-svg" width="140" height="240" viewBox="0 0 140 240" fill="none">
      <defs>
        <linearGradient id="darkgrad1" x1="20" y1="30" x2="120" y2="200" gradientUnits="userSpaceOnUse">
          <stop offset="0%" stopColor="#1a2030"/><stop offset="100%" stopColor="#0b1020"/>
        </linearGradient>
        <linearGradient id="darkgrad2" x1="38" y1="20" x2="102" y2="34" gradientUnits="userSpaceOnUse">
          <stop offset="0%" stopColor="#1e3060"/><stop offset="100%" stopColor="#0a1830"/>
        </linearGradient>
        <linearGradient id="darkgrad3" x1="55" y1="200" x2="85" y2="228" gradientUnits="userSpaceOnUse">
          <stop offset="0%" stopColor="#1e3060"/><stop offset="100%" stopColor="#0a1830"/>
        </linearGradient>
      </defs>
      ...
      {[65,80,95].map((y,i)=>(
        <line key={i} x1="35" y1={y} x2="105" y2={y} stroke={`rgba(100,160,255,${0.3-i*0.07})`} strokeWidth="1.5" strokeLinecap="round"/>
      ))}
...
function ModelHydro() {
  return (
    <svg className="ro-model-svg" width="200" height="200" viewBox="0 0 200 200" fill="none">
      <defs>
        <linearGradient id="slategrad" x1="10" y1="60" x2="190" y2="140" gradientUnits="userSpaceOnUse">
          <stop offset="0%" stopColor="#e8f0ff"/><stop offset="100%" stopColor="#c8daff"/>
        </linearGradient>
        <linearGradient id="cylgrad1" x1="18" y1="78" x2="62" y2="122" gradientUnits="userSpaceOnUse">
          <stop offset="0%" stopColor="#c0d8ff"/><stop offset="100%" stopColor="#8ab0ff"/>
        </linearGradient>
      </defs>
...
function ModelObsidian() {
  return (
    <svg className="ro-model-svg" width="130" height="260" viewBox="0 0 130 260" fill="none">
      <defs>
        <linearGradient id="luxgrad" x1="35" y1="20" x2="95" y2="220" gradientUnits="userSpaceOnUse">
          <stop offset="0%" stopColor="#141c30"/><stop offset="100%" stopColor="#060a14"/>
        </linearGradient>
      </defs>
      ...
      {[78,94,110].map((y,i)=>(
        <line key={i} x1="43" y1={y} x2="87" y2={y} stroke={`rgba(100,140,255,${0.15-i*0.03})`} strokeWidth="1.5" strokeLinecap="round"/>
      ))}
...
function DropsCanvas() {
  ...
  useEffect(() => {
    ...
    const anim = () => {
      ...
      dropsRef.current = dropsRef.current.filter(d => d.life > 0.01);
      dropsRef.current.forEach(d => {
        d.vy += 0.06; d.x += d.vx; d.y += d.vy; d.life -= 0.008;
        ctx.beginPath();
        ctx.ellipse(d.x, d.y, d.r * 0.6, d.r, 0, 0, Math.PI * 2);
        ctx.fillStyle = `rgba(0,87,255,${d.a * d.life})`;
        ctx.fill();
        if (d.y > canvas.height - 20) d.life = 0;
      });
      rafRef.current = requestAnimationFrame(anim);
    };
    anim();
    return () => { clearInterval(iv); cancelAnimationFrame(rafRef.current); window.removeEventListener("resize", resize); };
  }, []);

  return <canvas ref={canvasRef} className="drop-canvas" />;
}
...
function Footer() {
  ...
  useEffect(() => {
    ...
    const spawnDrop = () => {
      ...
      dropsRef.current.push({ x, y: -20, vy: 1.5 + Math.random() * 2, r: 3 + Math.random() * 4, a: 0.8, done: false });
    };
    const iv = setInterval(spawnDrop, 600);
    ...
    const anim = () => {
      ...
      dropsRef.current.forEach(d => {
        ...
        if (d.y >= poolY && !d.done) {
          d.done = true;
          ringsRef.current.push({ x: d.x, y: poolY, r: 2, maxR: 60 + Math.random() * 40, a: 0.6, speed: 1.5 + Math.random() });
          // Splash particles
          for (let s = 0; s < 6; s++) {
            ...
            dropsRef.current.push({
              ...
              r: 1.5 + Math.random() * 2,
              a: 0.6, splash: true, life: 1, done: false
            });
          }
        }
      });
      ...
      dropsRef.current = dropsRef.current.filter(d => {
        ...
        d.vy += 0.1; d.x += d.vx; d.y += d.vy; d.life -= 0.04;
      ...
      <div className="footer-content">
        <div className="footer-brand">
          ...
          <div className="footer-socials">
            {["𝕏","in","f","▶"].map((s, i) => (
              <div key={i} className="social-dot">{s}</div>
            ))}
          </div>
        </div>
        ...
        {Object.entries(FOOTER_LINKS).map(([cat, links]) => (
          <div key={cat} className="footer-col">
            ...
            {links.map(l => <a key={l} className="footer-link">{l}</a>)}
          </div>
        ))}
      </div>
    ...
    <div className={`prod-card reveal ${p.dark ? "dark-card" : ""}`}>
      ...
      <div className="prod-body">
        ...
        <div className="prod-specs">
          {p.specs.map((s, i) => (
            <div key={i} className="spec-row"><span className="check">✦</span> {s}</div>
          ))}
        </div>
  ...
  return (
    <div>
      <div className="products-hero">
        ...
        <p className="page-sub">Two models. One obsession. Choose the AquaPura that fits your life — both redefine what pure water means.</p>
      </div>
            ...
            <div key={i} className="comp-row">
              ...
              {row.vals.map((v, j) => (
                <div key={j} className="comp-cell">
                  {v === true ? <span className="comp-check">✓</span>
                   : v === false ? <span className="comp-dash">—</span>
                   : v}
                </div>
              ))}
            </div>
          ...
          {submitted ? (
            ...
            <div className="contact-form">
              ...
              <div className="form-row">
                ...
                <textarea rows="3" placeholder="Tell us about your home, family size, or any water quality concerns..." />
              </div>
      ...
      <div className="map-section">
        ...
        <div className="map-inner">
          <div style={{ textAlign: "center" }}>
            ...
            <p style={{ marginTop: "0.5rem", color: "var(--muted)", fontSize: "0.9rem" }}>AquaPura Experience Centre, Whitefield, Bengaluru</p>
            <button className="btn-primary" style={{ marginTop: "1.2rem", fontSize: "0.82rem", padding: "0.7rem 1.5rem" }}
              onClick={() => window.open("https://maps.google.com", "_blank")}>Open in Maps</button>
          </div>
        </div>
      </div>
...
export default function AquaPura() {
  ...
  const pages = { home: <HomePage navigate={navigate} tds={tds} />, about: <AboutPage />, products: <ProductsPage navigate={navigate} />, contact: <ContactPage /> };
    ...
    <>
      ...
      {mobileOpen && (
        <div className="mobile-menu open">
          {NAV_LINKS.map(p => (
            <a key={p} onClick={() => navigate(p)}>{p.charAt(0).toUpperCase() + p.slice(1)}</a>
          ))}
        </div>
      )}
